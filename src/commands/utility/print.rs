...
impl BotCommand for PrintCommand {
    ...
    fn examples(&self) -> &'static [&'static str] {
        &[
            ...
            ".print https://github.com/Mikasuru/Kurinium/doc.pdf \"HP LaserJet Pro\"",
...
impl PrintCommand {
    async fn show_help(&self, http: &Arc<HttpClient>, msg: &Message) -> Result<()> {
        let embed = EmbedBuilder::new()
            ...
            .footer(EmbedFooterBuilder::new("Kurinium Print Command"))
            .build();
    ...
    async fn handle_attachment(
        ...
        if msg.attachments.is_empty() {
            http.create_message(msg.channel_id)
                .content("**Error**: No file attached. Please attach a file.")
                .await?;
            return Ok(());
        }
        ...
        if attachment.size > MAX_FILE_SIZE {
            http.create_message(msg.channel_id)
                .content(&format!("**Error**: File is too large ({}MB). Max: {}MB", 
                    attachment.size / 1024 / 1024, MAX_FILE_SIZE / 1024 / 1024))
                .await?;
            return Ok(());
        }
        ...
        if let Some(ext) = Path::new(&attachment.filename).extension() {
            ...
            if !ALLOWED_EXTENSIONS.contains(&ext.as_str()) {
                http.create_message(msg.channel_id)
                    .content(&format!("**Error**: File type '{}' not supported. Allowed: {}", 
                        ext, ALLOWED_EXTENSIONS.join(", ")))
                    .await?;
                return Ok(());
            }
        ...
        if msg.attachments.len() > 1 {
            http.create_message(msg.channel_id)
                .content(&format!("**Note**: Printing only first file: {}", attachment.filename))
                .await?;
        }
    ...
    async fn print_file_path(
        ...
        match self.print_file_raw(&path, printer) {
            ...
            Err(e) => {
                http.create_message(msg.channel_id)
                    .content(&format!("**Error**: Failed to print: {}", e))
                    .await?;
            }
        }
    ...
    async fn print_from_url(
        ...
        if let Some(ext) = Path::new(file_name).extension() {
            ...
            if !ALLOWED_EXTENSIONS.contains(&ext.as_str()) {
                http.create_message(msg.channel_id)
                    .content(&format!("**Error**: File type '{}' not supported", ext))
                    .await?;
                return Ok(());
            }
        }
    ...
    async fn download_and_print(
        ...
        let response = match timeout(DOWNLOAD_TIMEOUT, client.get(url).send()).await {
            ...
            Ok(Err(e)) => {
                http.create_message(msg.channel_id)
                    .content(&format!("**Error**: Download failed: {}", e))
                    .await?;
                return Ok(());
            }
            Err(_) => {
                http.create_message(msg.channel_id)
                    .content("**Error**: Download timeout")
                    .await?;
                return Ok(());
            }
        };
        ...
        if !response.status().is_success() {
            http.create_message(msg.channel_id)
                .content(&format!("**Error**: HTTP {}", response.status()))
                .await?;
            return Ok(());
        }
        ...
        if let Some(len) = response.content_length() {
            if len > MAX_FILE_SIZE {
                http.create_message(msg.channel_id)
                    .content(&format!("**Error**: File too large ({}MB)", len / 1024 / 1024))
                    .await?;
                return Ok(());
            }
        }
        ...
        let bytes = match response.bytes().await {
            ...
            Err(e) => {
                http.create_message(msg.channel_id)
                    .content(&format!("**Error**: Failed to read content: {}", e))
                    .await?;
                return Ok(());
            }
        };
        ...
        let temp_path = std::env::temp_dir()
            .join(format!("kurinium_print_{}_{}", msg.id.get(), file_name));
